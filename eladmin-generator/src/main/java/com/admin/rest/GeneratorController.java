...
public class GeneratorController {
    ...
    public ResponseEntity<Object> queryColumns(@RequestParam String tableName){
        List<ColumnInfo> columnInfos = columnInfoService.getColumns(tableName);
    ...
    public ResponseEntity<HttpStatus> save(@RequestBody List<ColumnInfo> columnInfos){
        columnInfoService.saveOrUpdateBatch(columnInfos);
    ...
    public ResponseEntity<HttpStatus> sync(@RequestBody List<String> tables){
        for (String table : tables) {
            columnInfoService.sync(columnInfoService.getColumns(table), columnInfoService.queryByTableName(table));
        }
        return new ResponseEntity<>(HttpStatus.OK);
    }
    ...
    public ResponseEntity<Object> generator(@PathVariable String tableName, @PathVariable Integer type, HttpServletRequest request, HttpServletResponse response){
        ...
        switch (type){
            // 生成代码
            case 0: generatorService.generator(genConfigService.find(tableName), columnInfoService.getColumns(tableName));
                    break;
            // 预览
            case 1: return generatorService.preview(genConfigService.find(tableName), columnInfoService.getColumns(tableName));
            // 打包
            case 2: generatorService.download(genConfigService.find(tableName), columnInfoService.getColumns(tableName), request, response);
                    break;
