...
import com.admin.modules.mnt.service.mapstruct.MapStructDeployMapper;
...
public class DeployServiceImpl extends ServiceImpl<DeployMapper,Deploy> implements DeployService {
	...
	private final MapStructDeployMapper mapStructDeployMapper;
	...
	public Object queryAll(DeployQueryCriteria criteria, IPage pageable) {
		QueryWrapper<Deploy> queryWrapper = (QueryWrapper<Deploy>) QueryHelp.getQueryWrapper(criteria, Deploy.class);
		IPage<Deploy> page = this.page(pageable, queryWrapper);
		List<DeployDto> collect = page.getRecords().stream().map(mapStructDeployMapper::toDto).collect(Collectors.toList());
	...
	public List<DeployDto> queryAll(DeployQueryCriteria criteria) {
		QueryWrapper<Deploy> queryWrapper = (QueryWrapper<Deploy>) QueryHelp.getQueryWrapper(criteria, Deploy.class);
		List<Deploy> list = this.list(queryWrapper);
		return list.stream().map(mapStructDeployMapper::toDto).collect(Collectors.toList());
	}

	@Override
	public DeployDto findById(Long id) {
		Deploy deploy = this.getById(id);
		if(ObjectUtil.isNull(deploy)){
			deploy = new Deploy();
		}
		ValidationUtil.isNull(deploy.getId(), "Deploy", "id", id);
		return mapStructDeployMapper.toDto(deploy);
	}
	...
	public void update(Deploy resources) {
		Deploy deploy = this.getById(resources.getId());
		if(ObjectUtil.isNull(deploy)){
			deploy = new Deploy();
		}
